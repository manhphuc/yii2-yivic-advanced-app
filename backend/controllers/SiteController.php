...
use yivic\yivicCms\libs\override\web\NpController;
use yii;
...
class SiteController extends NpController
{
	...
	public function actionLogin()
	{
		...
		if ($msLoginForm->load(Yii::$app->request->post()) && $msLoginForm->login($msLoginForm->loginType)) {
			...
			$this->layout = '@yivic/yivicCms/views/layouts/backend/login';

			// Reset password to blank when returning from submit
			$msLoginForm->password = '';
			return $this->render('@yivic/yivicCms/views/backend/site/login', [
	...
	{
		$url = Yii::$app->getUrlManager()->createUrl('/yivicCms/backend/site/logout');
